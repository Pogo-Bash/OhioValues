...
const currentSide = ref('your'); // 'your' or 'their'
...
const parsePrice = (price) => {
  if (price === '?') return 0; // Handle unknown prices
...
const filteredWeapons = computed(() => {
  let filtered = weapons.value.filter(w => w.price !== '?'); // Filter out unknown prices
...
const yourTotal = computed(() => {
  return yourOffer.value.reduce((total, item) => total + parsePrice(item.price), 0);
});

const theirTotal = computed(() => {
  return theirOffer.value.reduce((total, item) => total + parsePrice(item.price), 0);
});

const tradeDifference = computed(() => {
  return yourTotal.value - theirTotal.value;
});

const tradeResult = computed(() => {
  const diff = Math.abs(tradeDifference.value);
  const maxTotal = Math.max(yourTotal.value, theirTotal.value);
  
  if (maxTotal === 0) {
    return { type: 'empty', message: '⚖️ Add items to compare', class: 'text-purple-400' };
  }
  
  const percentage = diff / maxTotal * 100;
  ...
  if (tradeDifference.value === 0) {
    ...
    return { type: 'you-lose', message: '📉 You\'re Overpaying', class: 'text-red-400' };
  } else {
    return { type: 'you-win', message: '📈 You\'re Getting a Good Deal!', class: 'text-green-400' };
  }
});
...
const addWeapon = (weapon) => {
  ...
  if (currentSide.value === 'your') {
    yourOffer.value.push(weaponCopy);
  } else {
    theirOffer.value.push(weaponCopy);
  }
...
const removeWeapon = (side, uniqueId) => {
  if (side === 'your') {
    yourOffer.value = yourOffer.value.filter(w => w.uniqueId !== uniqueId);
  } else {
    theirOffer.value = theirOffer.value.filter(w => w.uniqueId !== uniqueId);
  }
};
...
<template>
  <div class="min-h-screen bg-gradient-to-b from-gray-900 via-purple-900/20 to-black text-white">
    <!-- Navigation -->
    <nav class="fixed top-0 left-0 w-full backdrop-blur-md bg-black/50 border-b border-purple-800 shadow-lg flex justify-between items-center px-6 py-4 z-40">
      <button @click="props.showHome" class="text-2xl font-bold text-purple-300 hover:text-white transition">
        Stack's Ohio Values
      </button>
      <!-- Desktop Menu -->
      <div class="hidden lg:flex space-x-6">
        <button @click="props.showHome" class="text-purple-300 hover:text-white transition">Home</button>
        <span class="text-white font-semibold">Calculator</span>
      ...
      <div v-if="isMenuOpen" class="absolute top-16 right-6 bg-black/50 backdrop-blur-lg border border-purple-700 shadow-xl rounded-xl p-4 space-y-3 w-48 lg:hidden">
        <button @click="props.showHome(); isMenuOpen = false" class="block w-full text-left px-4 py-2 text-purple-300 hover:bg-purple-700 hover:text-white rounded transition">
          Home
        </button>
        <span class="block px-4 py-2 text-white bg-purple-700 rounded">Calculator</span>
    ...
    <div class="pt-20">
      <div class="container mx-auto px-4 md:px-6 py-8">
        ...
        <div class="text-center mb-8">
          <h1 class="text-3xl md:text-4xl font-bold text-purple-300 mb-4">⚖️ Trade Calculator</h1>
          <p class="text-purple-200">Compare weapon skin values to check if your trade is fair</p>
        </div>

        <!-- Trade Result -->
        <div class="mb-8">
          <div class="bg-gray-900/50 border border-purple-700 rounded-xl p-6 text-center">
            <div class="text-2xl md:text-3xl font-bold mb-2" :class="tradeResult.class">
              {{ tradeResult.message }}
            </div>
            <div v-if="tradeDifference !== 0 && yourTotal > 0 && theirTotal > 0" class="text-purple-300 mb-2">
              Difference: {{ formatPrice(Math.abs(tradeDifference)) }}
              {{ tradeDifference > 0 ? ' (You\'re giving more)' : ' (You\'re receiving more)' }}
            </div>
            <div class="flex flex-col sm:flex-row gap-4 justify-center items-center mt-4">
              <div class="text-sm text-purple-400">
                Your Offer: <span class="text-blue-400 font-bold">{{ formatPrice(yourTotal) }}</span>
                vs
                Their Offer: <span class="text-green-400 font-bold">{{ formatPrice(theirTotal) }}</span>
              </div>
        ...
        <!-- Trade Comparison - Mobile Optimized -->
        <div class="space-y-6">
          <!-- Your Offer -->
          <div class="bg-gray-900/50 border-2 border-blue-600 rounded-xl p-4 md:p-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
              <h2 class="text-xl md:text-2xl font-bold text-blue-400">📦 Your Offer</h2>
              <div class="flex gap-2 flex-wrap">
                <button @click="clearSide('your')" class="text-red-400 hover:text-red-300 text-sm px-2 py-1">
                  Clear
                </button>
                <button @click="openAddModal('your')" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg transition text-white font-semibold text-sm md:text-base">
                  ➕ Add Item
                </button>
              </div>
            </div>
            ...
            <!-- Your Offer Items -->
            <div class="space-y-3 mb-4 min-h-[120px]">
              <div v-if="yourOffer.length === 0" class="text-center text-purple-400 py-8">
                <div class="text-4xl mb-2">📭</div>
                <p>Click "Add Item" to start building your offer</p>
              </div>
              
              <div v-for="weapon in yourOffer" :key="weapon.uniqueId" 
                   class="bg-gray-800/50 border border-purple-600 rounded-lg p-3 md:p-4 flex justify-between items-center">
                <div class="flex items-center space-x-3 min-w-0 flex-1">
                  <span class="px-2 py-1 rounded text-xs font-semibold flex-shrink-0" :class="getRarityColor(weapon.category)">
                    {{ weapon.category }}
                  </span>
                  <div class="min-w-0 flex-1">
                    <div class="font-semibold text-white truncate">{{ weapon.weapon }}</div>
                    <div class="text-green-400 font-bold">💵 {{ weapon.price }}</div>
                  </div>
                </div>
                <button @click="removeWeapon('your', weapon.uniqueId)" 
                        class="text-red-400 hover:text-red-300 p-2 flex-shrink-0">
                  ❌
                </button>
              </div>
            </div>
            ...
            <div class="border-t border-purple-600 pt-4">
              <div class="text-lg md:text-xl font-bold text-blue-400">
                Total: {{ formatPrice(yourTotal) }}
              </div>
            </div>
          </div>

          <!-- Their Offer -->
          <div class="bg-gray-900/50 border-2 border-green-600 rounded-xl p-4 md:p-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
              <h2 class="text-xl md:text-2xl font-bold text-green-400">🎁 Their Offer</h2>
              <div class="flex gap-2 flex-wrap">
                <button @click="clearSide('their')" class="text-red-400 hover:text-red-300 text-sm px-2 py-1">
                  Clear
                </button>
                <button @click="openAddModal('their')" class="bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg transition text-white font-semibold text-sm md:text-base">
                  ➕ Add Item
                </button>
              </div>
            </div>
            ...
            <!-- Their Offer Items -->
            <div class="space-y-3 mb-4 min-h-[120px]">
              <div v-if="theirOffer.length === 0" class="text-center text-purple-400 py-8">
                <div class="text-4xl mb-2">📭</div>
                <p>Click "Add Item" to add their offer</p>
              </div>
              
              <div v-for="weapon in theirOffer" :key="weapon.uniqueId" 
                   class="bg-gray-800/50 border border-purple-600 rounded-lg p-3 md:p-4 flex justify-between items-center">
                <div class="flex items-center space-x-3 min-w-0 flex-1">
                  <span class="px-2 py-1 rounded text-xs font-semibold flex-shrink-0" :class="getRarityColor(weapon.category)">
                    {{ weapon.category }}
                ...
                <button @click="removeWeapon('their', weapon.uniqueId)" 
                        class="text-red-400 hover:text-red-300 p-2 flex-shrink-0">
                  ❌
                </button>
              </div>
            ...
            <!-- Their Offer Total -->
            <div class="border-t border-purple-600 pt-4">
              <div class="text-lg md:text-xl font-bold text-green-400">
                Total: {{ formatPrice(theirTotal) }}
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div class="bg-gray-900 border border-purple-600 rounded-xl p-4 md:p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-start mb-6">
          <h3 class="text-xl md:text-2xl font-bold text-purple-300">
            ➕ Add Item to {{ currentSide === 'your' ? 'Your Offer' : 'Their Offer' }}
          </h3>
          <button @click="showAddModal = false" class="text-purple-400 hover:text-white text-2xl">
            &times;
          </button>
        </div>
        ...
        <div class="mb-6 flex flex-col gap-4">
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            <select v-model="selectedCategory" class="flex-1 px-4 py-3 bg-gray-800 border border-purple-600 rounded-lg text-white focus:border-purple-400 focus:outline-none">
              <option value="">All Categories</option>
              <option v-for="category in categories" :key="category" :value="category">{{ category }}</option>
            </select>
            
            <select v-model="selectedAvailability" class="flex-1 px-4 py-3 bg-gray-800 border border-purple-600 rounded-lg text-white focus:border-purple-400 focus:outline-none">
        ...
        <!-- Weapons List -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[400px] overflow-y-auto">
          <div v-for="weapon in filteredWeapons" :key="weapon.id" 
            ...
            <!-- Weapon Info -->
            <div>
              <h4 class="text-lg font-bold text-white mb-2 truncate">{{ weapon.weapon }}</h4>
